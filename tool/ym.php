    ...
    function getCode () {
        if ($_POST['url'] != null) {
            ...
            if ($curl['Info']['redirect_url'] != '') {
                $curl = Curl($curl['Info']['redirect_url']);
                if ($curl['Info']['redirect_url'] == '') {
                    $curlResultTagInfo = strip_tags($curl['Result'], "<title>");
                    echo "<script>console.log('重定向:ok:".$curl['Info']['redirect_url']."');</script>";
                    return ['result' => $curl['Result'], 'tags' => $curlResultTagInfo, 'info' => $curl['Info']];
                } else {
                    $curl = Curl($curl['Info']['redirect_url']);
                    $curlResultTagInfo = strip_tags($curl['Result'], "<title>");
                    echo "<script>console.log('重定向:no".$curl['Info']['redirect_url']."');</script>";
                    return ['result' => $curl['Result'], 'tags' => $curlResultTagInfo, 'info' => $curl['Info']];
                }
            } else {
                $curlResultTagInfo = strip_tags($curl['Result'], "<title>");
                echo "<script>console.log('重定向:no');</script>";
                return ['result' => $curl['Result'], 'tags' => $curlResultTagInfo, 'info' => $curl['Info']];
            }
        }
    }
...
<body>
    ...
    <div class="searchInfo">
        <form action="./ym.php" method="post">
            <input type="text" placeholder="网页地址" class="searchInput" name="url" id="info">
            <button class="searchButton" name="searchButton">获取</button>
        </form>
    </div>

    <div class="display-none">
        <?php
            if ($getCode['tags'] != null) {
                echo $getCode['tags'];
            }
        ?>
    </div>
    ...
    <div style="width: 70%; margin: 0 auto; padding: 10px; margin-bottom: 20px; background-color: #f5f5f5;" id="xinXiFenXi">
        ...
        <p style="margin-top: 10px; font-size: 0.8em; color: #a03434;" id="titleInfoText">...</p>
        <p style="margin-top: 10px; font-size: 0.8em; color: #666;" id="pageInfoText">...</p>
        <p style="margin-top: 10px; font-size: 0.8em; color: #666;" id="sizeInfoText">...</p>
        <p style="margin-top: 10px; font-size: 0.8em; color: #666;" id="reloadInfoText">...</p>
        <p style="margin-top: 10px; font-size: 0.8em; color: #666;" id="speedInfoText">...</p>
        <p style="margin-top: 10px; font-size: 0.8em; color: #666;" id="ipInfoText">...</p>

        <?php
            // 如果标签信息有数据
            if ($getCode['tags'] != null) {
                echo "<script>
                    var titleInfo = document.getElementsByTagName('title')[1].innerText;
                    titleInfoText.innerText = '<title>' + titleInfo + '</title>';
                </script>";
            }
            ...
            // if ($getNewCurlInfo) {
            //     echo "<script>
            //         pageInfoText.innerText = '页面类型:".$getNewCurlInfo['content_type']."';
            //     </script>";
            // }

            if ($getCode['info'] != null) {
                echo "<script>
                    pageInfoText.innerText = '页面类型: ".$getCode['info']['content_type']."';
                </script>";

                echo "<script>
                    sizeInfoText.innerText = '页面大小: ".$getCode['info']['size_download']." byte';
                </script>";

                echo "<script>
                    speedInfoText.innerText = '下载速度: ".$getCode['info']['speed_download']." kb/s or ".($getCode['info']['speed_download'] / 1000)." mb/s';
                </script>";

                $suiJiIP = "222.222.".mt_rand(0, 254).".".mt_rand(0, 254);

                echo "<script>
                    ipInfoText.innerText = '代理IP: ".$suiJiIP.":80';
                </script>";

                if ($getCode['info']['redirect_url'] != null) {
                    echo "<script>
                        reloadInfoText.innerText = '重定向: ".$getCode['info']['redirect_url']."';
                    </script>";
                } else {
                    echo "<script>
                        reloadInfoText.innerText = '重定向: 已自动过滤两层重定向';
                    </script>";
                }
            }

            // 如果大小信息有数据
            // if ($curlResult['Result'] != null) {
            //     echo "<script>
            //         sizeInfoText.innerText = '页面大小:".$newResult['size_download']." byte';
            //     </script>";
            // }
        ?>
